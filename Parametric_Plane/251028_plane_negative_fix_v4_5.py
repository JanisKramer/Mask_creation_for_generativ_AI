#!/usr/bin/env python3
"""
Street Bands — v4.5 (top-edge clip only; unlimited left/right)

Changes vs v4.4:
- Fixes "left side can't be created": we no longer clip overlays to the entire plane polygon.
  Instead, we only clip **above the plane's top edge** (TL→TR line) to prevent sky doubling,
  while allowing bands to extend freely left/right across the full image.
- Keeps baseline midpoint recentring on a very large canvas so negatives have room.
"""
...
class StreetBands:
    def __init__(self):
        ...
        self.clicks_baseline: List[Tuple[float, float]] = []
        self.clicks_ref: List[Tuple[float, float]] = []

        self.bands: List[Band] = []
        self.invert_normal: bool = False

        self.pad_x, self.pad_y = 3000, 3000
        self.top_w, self.top_h = 20000, 12000
        ...
        self.label_top: Optional[np.ndarray] = None
        self.ref_cm: float = 100.0

        self.clip_below_top_img: Optional[np.ndarray] = None

    def load_image(self, img: np.ndarray, img_path: str | None = None):
        self.__init__()
        self.img_bgr = img.copy()
        self.img_path = img_path
        return to_rgb_uint8(self.img_bgr), "Click 4 points (BL, TL, TR, BR)."
    ...
    def click_image(self, x: float, y: float):
        ...
        if self.mode == 'plane':
            ...
            if len(self.clicks_plane) < 4:
                return gr.update(), f"Plane clicks: {len(self.clicks_plane)}/4..."
            return self.lock_plane()
        if self.mode == 'baseline':
            self.clicks_baseline.append((x, y))
            if len(self.clicks_baseline) < 2:
                return self.preview(status="Baseline: 1/2 points.")
            self.mode = 'reference'
            return self.preview(status="Baseline set. Now click 2 reference points (known length).")
        if self.mode == 'reference':
            self.clicks_ref.append((x, y))
            if len(self.clicks_ref) < 2:
                return self.preview(status="Reference: 1/2 points.")
    ...
    def lock_plane(self):
        ...
        # Build "below top edge only" clip mask
        tl = self.quad_img_extended[1]; tr = self.quad_img_extended[2]
        x0, x1 = 0.0, float(w - 1)
        if abs(tr[0] - tl[0]) < 1e-6:
            y_left = min(tl[1], tr[1])
            y_right = y_left
        else:
            m = (tr[1] - tl[1]) / (tr[0] - tl[0])
            b = tl[1] - m * tl[0]
            y_left = m * x0 + b
            y_right = m * x1 + b
        y_left = float(np.clip(y_left, 0, h - 1))
        y_right = float(np.clip(y_right, 0, h - 1))

        poly = np.array([[0, h - 1], [0, y_left], [w - 1, y_right], [w - 1, h - 1]], dtype=np.int32)
        self.clip_below_top_img = np.zeros(self.img_bgr.shape[:2], dtype=np.uint8)
        cv2.fillPoly(self.clip_below_top_img, [poly], 255)
    ...
    def lock_baseline_and_reference(self, bx0: float, by0: float, bx1: float, by1: float,
                                    rx0: float, ry0: float, rx1: float, ry1: float, ref_cm: float):
        if self.H is None:
        ...
        mid = (b0t + b1t) * 0.5
        dx = (self.top_w * 0.5) - float(mid[0])
        dy = (self.top_h * 0.5) - float(mid[1])
        T = np.array([[1.0, 0.0, dx], [0.0, 1.0, dy], [0.0, 0.0, 1.0]], dtype=np.float64)
        self.H = T @ self.H
        self.Hinv = np.linalg.inv(self.H); self.Hinv /= (self.Hinv[2, 2] + EPS)
        ...
        self.mode = 'bands'
        return self.refresh_overlay(status=f"Baseline + scale locked (1 px ≈ {self.m_per_px:.4f} m). Negatives go left.")
    ...
    def add_band(self, label: str, from_cm: float, to_cm: float):
        ...
        a_cm = float(from_cm); b_cm = float(to_cm)
        if a_cm == b_cm:
            return self.preview(status="Band has zero width.")
        a_px = (a_cm / 100.0) / self.m_per_px
        b_px = (b_cm / 100.0) / self.m_per_px
        ...
        d_lo, d_hi = (a_px, b_px) if a_px <= b_px else (b_px, a_px)

        L = max(seg_len(v) + 8000.0, 12000.0)
        u = vn * L * 0.5
        c = (b0 + b1) * 0.5
        p0 = c - u; p1 = c + u

        q0 = p0 + n * d_lo; q1 = p1 + n * d_lo
        r0 = p0 + n * d_hi; r1 = p1 + n * d_hi
        poly = np.array([q0, q1, r1, r0], dtype=np.float32).round().astype(np.int32)

        if self.label_top is None:
            self.label_top = np.zeros((self.top_h, self.top_w), dtype=np.int32)

        band_idx = len(self.bands) + 1
        mask = np.zeros((self.top_h, self.top_w), dtype=np.uint8)
        cv2.fillPoly(mask, [poly], 255)
        if mask.max() == 0:
            return self.refresh_overlay(status="Band fell outside the canvas. Try toggling 'Invert normal' or check distances.")

        self.label_top[mask > 0] = band_idx
        color = PALETTE[(band_idx - 1) % len(PALETTE)]
        self.bands.append(Band(label=label, from_cm=a_cm, to_cm=b_cm, color_rgb=color))
        return self.refresh_overlay(status=f"Added band '{label}' [{a_cm:.1f}–{b_cm:.1f} cm]. (Negatives = left)")
    ...
    def _warp_labels_to_image(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.clip_below_top_img is not None:
            clip3 = np.repeat((self.clip_below_top_img > 0)[:, :, None], 3, axis=2)
            overlay = np.where(clip3, overlay, 0)
            label_img = np.where(self.clip_below_top_img > 0, label_img, 0).astype(np.int32)
    ...
    def set_invert_normal(self, flag: bool):
        ...
        side = "right" if self.invert_normal else "left"
        return self.preview(status=f"Invert normal: {'ON' if self.invert_normal else 'OFF'} (positive distances go to the {side}).")
...
with gr.Blocks(title="Street Bands v4.5 — top-edge clip only", analytics_enabled=False) as demo:
    gr.Markdown("## Street Bands v4.5 — top-edge clip only\\n"
                "Clip above the horizon; allow left/right extension.")
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            with gr.Row():
                ...
                invert_toggle = gr.Checkbox(label="Invert normal (swap positive side)", value=False)
            ...
            gr.Markdown("### Bands (cm) — negatives allowed")
            with gr.Row():
                band_label = gr.Textbox(label="Label", value="left_lane")
                from_cm = gr.Number(label="from_cm", value=-200)
                to_cm = gr.Number(label="to_cm", value=0)
            with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            def refresh_side_action():
                ...
                return _json.dumps(band_table_state(), indent=2)
